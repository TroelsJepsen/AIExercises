...
def reproduce(mother, father):
    ...
    child = []
    child.extend(mother[:crossover_point])
    child.extend(father[crossover_point:])
    return tuple(child)
    #return child
...
def mutate(individual):
    ...
    bit_to_mutate = random.randint(0, len(individual)-1)
    mutant[bit_to_mutate] = (mutant[bit_to_mutate]-1) *-1
    return tuple(mutant)
...
def random_selection(population, fitness_fn):
    ...
    for individual in ordered_population:
        total_fitness += fitness_function(individual)
    while len(selected) < 2:
        for individual in ordered_population:
            if random.randint(0,100) < (fitness_function(individual) / total_fitness) *100:
...
def fitness_function(individual):
    ...
    fitness = 0
    position_bit_pairs = enumerate(reversed(individual))
    for pair in position_bit_pairs:
