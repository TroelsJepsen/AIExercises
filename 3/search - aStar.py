class Node:  # Node has only PARENT_NODE, STATE, DEPTH
    def __init__(self, state, hfunction, pathcost, parent=None):
        self.STATE = state
    ...
    def __repr__(self):
        return 'State: ' + str(self.STATE) + ' - Heuristic function value: ' + str(self.HFUNCTION) + ' - Cost so far ' + str(self.PATHCOST)
...
def TREE_SEARCH():
    #fringe = []
    current_node = Node(INITIAL_STATE,INITIAL_HEURISTIC_FUNCTION, 0)
    ...
    #REMOVE_FIRST(fringe)
        ...
        if not children:
            ...
            for x in range(0, noOfChildren-1):
                if x ==0 :
                    current_node = children[0]
                else:
                    current_node = compare_children(children[x-1], children[x])
...
'''
Compares desirability of states at different child nodes
'''
def compare_children(child1, child2):
    if (child1.PATHCOST + child1.HFUNCTION * 1000000000000) < (child2.PATHCOST + child2.HFUNCTION * 1000000000000):
        return child1
    else:
        return child2
